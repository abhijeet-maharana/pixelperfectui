...
-- Insert sample blog posts using a more efficient approach
WITH numbered_rows AS (
  SELECT 
    ...
    (ARRAY[
      ...
      'Landing Page Ideas',
      'Mobile UI/UX'
    ])[1 + mod(generate_series(1, 30) - 1, 4)] as category,
    (ARRAY[
      'https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&q=80&w=2000',
      'https://images.unsplash.com/photo-1492551557933-34265f7af79e?auto=format&fit=crop&q=80&w=2000',
      'https://images.unsplash.com/photo-1512486130939-2c4f79935e4f?auto=format&fit=crop&q=80&w=2000'
    ])[1 + mod(generate_series(1, 30) - 1, 3)] as cover_image,
    (SELECT id FROM blog_authors WHERE name = CASE 
      WHEN mod(generate_series(1, 30), 2) = 0 THEN 'Sarah Chen'
      ELSE 'Alex Thompson'
    END) as author_id
)
INSERT INTO blog_posts (
...
SELECT
  'Blog Post ' || n::text as title,
  'Sample description for blog post ' || n::text as description,
  '# Sample content...' as content,
  cover_image,
  category,
  ARRAY['Sample Tag 1', 'Sample Tag 2'] as tags,
  (5 + mod(n, 5))::text || ' min read' as reading_time,
  false as featured,
  author_id,
